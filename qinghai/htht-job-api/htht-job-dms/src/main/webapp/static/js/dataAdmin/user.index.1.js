...
$(function () {
    table=$("#table").dataTable({
        ...
        "ajax": {
            url: base_url + "/dbms_user/pageList",
            type: "post",
            data: function (d) {
                console.log(d)
                var obj = {};
    ...
    var addModalValidate = $("#addModal .form").validate({
        ...
        submitHandler : function(form) {
            $.post(base_url + "/dbms_user/saveUser",  $("#addModal .form").serialize(), function(data, status) {
                if (data.code == "200") {
                    $('#addModal').modal('hide');
                    layer.msg( "新增用户成功", {
    					icon: 1,
    ...
    var updateModalValidate = $("#updateModal .form").validate({
        ...
        rules : {
            ...
            updateAckPassword:{
                required:true,
                equalTo:"#updatePassword"    //新密码的id选择器
            }
        },
        messages : {
            userName : {
        ...
        submitHandler : function(form) {
            $("#updateModal .form input[name='password']").val($("#updateAckPassword").val());
            $.post(base_url + "/dbms_user/saveUser",  $("#updateModal .form").serialize(), function(data, status) {
                if (data.code == "200") {
...
function del(obj) {
    layer.confirm('确认删除?', {icon: 3, title:'系统提示'}, function(index){
        ...
        $.ajax({
            type: "post",
            dataType: "json",
            url: base_url+"/dbms_user/delUser/"+id,
            success: function(data) {
                if (data.code == 200) {
                	 layer.msg( "删除成功", {
     					icon: 1,
