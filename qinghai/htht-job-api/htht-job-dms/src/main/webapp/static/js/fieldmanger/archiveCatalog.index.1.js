...
//              $("#addArchiveRulesModal .form input[name='menuId']").val(menuId+arr[0].id);
    ...
    $("#enableBtn").click(function (data) {
        ...
        layer.confirm('确认启用?', {
            ...
            function(index) {
                ...
                if(selectedEnableItems.length > 0){
                    $.ajax({
                        type: "post",
                        dataType: "json",
                        url: base_url + "/archiveCatalog/enableOrdisable",
                        data:{data: JSON.stringify(selectedEnableItems),mark:0},
                        success: function(data) {
                            if (data.code === 200) {
                                layer.msg( "启用成功", {
                                    icon: 1,
  ...
  //规则禁用
            ...
            function(index) {
                ...
                if(selectedEnableItems.length > 0){
                    $.ajax({
                        ...
                        dataType: "json",
                        url: base_url + "/archiveCatalog/enableOrdisable",
                        data:{data: JSON.stringify(selectedEnableItems),mark:1},
                        success: function(data) {
                            if (data.code === 200) {
                                layer.msg( "禁用成功", {
                                    icon: 1,
    ...
    $("#updateDiskBtn").click(function() {
        ...
        if (selectedEnableItems1.length ==0){
            layer.open({
                title: '系统提示',
                content: ("请选择修改项"),
                icon: '2'
    ...
    $("#uodateDiskSubmit").click(function() {
        ...
        if(selectedEnableItems.length > 0){
            $.ajax({
                ...
                data:{ids: JSON.stringify(selectedEnableItems),archivdisk:$("#updateDiskModal .form select[name=archivdisk] option:selected").val()},
                success: function(data) {
                    if (data.code === 200) {
                        layer.msg( "修改成功", {
                            icon: 1,
