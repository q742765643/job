...
var httpUrls = {
	treeUrl : httpDateAndHeader.urlHeader + "/v1/api/index1",
	getDataUrl : httpDateAndHeader.urlHeader + "/v1/api/index2",
	saveFileUrl : httpDateAndHeader.urlHeader + "/v1/api/processDesigner/saveLocal",
	...
	getProcessXmlById : httpDateAndHeader.urlHeader +'/flow/getProcessXmlById',
	saveProcessFromDesigner : httpDateAndHeader.urlHeader + "/flow/saveProcessFromDesigner"
}
...
var page = {
	...
	init : function(isLocked){
		...
		if(page.istrue){
			...
			if(cell.id.indexOf("flowCell") >= 0){
				alert("您在并行组件内又重新放入了一个并行，这将可能导致位置显示不明的错误，您可以删除此并行框，或者调大父组件并行")
			}
		}
		cell = vertex;
		...
		cells && cells.map(function(x){
			...
			if(thisCellY <= distanceOnCell + cellY && thisCellY >= cellY){
                inCellUp = x;
			}
		})
		if(inCellUp){
		...
		if(overlays == null&&status==20){
			var overlay = new mxCellOverlay(new mxImage('/static/js/flow/images/warning.gif',16, 16), 'loading');
			overlay.cursor = 'hand';
		...
		if(status == 40){
			graph.stylesheet.styles[cell.getStyle()].gradientColor = '#FF0000';
			graph.refresh(cell);
		}
		...
		try {
			...
			var vStart = graph.insertVertex(parent, null, "开始", rWidth/2 - 40, 15, 80,30,"base");
			var centerX = vStart.geometry.getCenterX();
			var vEnd = graph.insertVertex(parent, null, '结束', rWidth/2 - 40, 280, 80,30,"base");
		...
		document.onkeydown = function(evt){
			...
			if(evt.keyCode && (evt.keyCode == 46) && graph.selectionModel){
				...
				if(cell && (cell.id == "begin" || cell.id == "vContain" || cell.id == "end")){
					mxUtils.error("组件不可被删除", 280,true);
				}else{
	...
	/*	if($(document).find(".info").html()){
		...
		if(cell.id.indexOf("flowCell") >= 0){
			...
			page.addLabel(graph,cell,"并行","label-flow-"+cell.id.split("_")[1]);
			graph.refresh(cell);
		}
		...
		graph.addListener(mxEvent.CELLS_ADDED,function(sender,evt){
			...
			if(cell != null && !cell.edge){				
				if(cell.id.indexOf('sequence') >= 0){
					...
					page.addLabel(graph,cell,"串行","label-seq-"+cell.id.split("_")[1]);
					return;
				}
				if(cell.id.indexOf("flowCell") >= 0){
					page.defaultParent = cell;
...
function cellChangeH(cell,dy){
	...
	if(dy < 0){
		...
		if(cellHeight < minHeight){
			...
			graph.refresh(cell);
			$(document).alert("达到最小边界，已自动进行边界处理");			
		}
...
function getData(nodeId,ele,cell,cellBegin,isTrue){
	...
	var url = httpUrls.getDataUrl + "?id=" + nodeId;
	...
	$.getJSON(url,function(data){
		...
		arr = eval('('+ data.dynamicParameter+')');
		var typeId = data.typeId;
		console.log(typeId);
		cell.isProcess = typeId == 2 ? true : false;
		console.log(cell.isProcess);
 		outArr =eval('('+data.outputParameter+')');
		...
		outArr.map(function(x){
			var obj2 = {};
			var l = outArr.length;
			var date = isRandomRepeat(outArr.length,1000,saveRadomObjOutput);
			try{
                if(date == undefined){
                    date = isRandomRepeat(arr.length,1000,saveRadomObjOutput);
                }
				date = date && date.toString().length == 3 ? date : date + "a";							
			}
			catch(e){
            ...
            obj2.parameterDesc = x.parameterDesc;
            //obj2.parameterName = x.name;
            obj2.parameterName = x.parameterName;
            obj2.parameterType=x.parameterType;
	...
	var addContainNode = function(arr,ele,parEle,isAttr){			
		if(isAttr){					
			...
			arr.map(function(x){						
				...
				obj.parameterType = x.parameterType;
	...
	return {
		init : function(){
			...
			cells.map(function(x){
				...
				else if(cellID.indexOf("a") >= 0){					//中间节点
					FigureType = "Service";
					FigureId = cellID;
                    Figure.setAttribute("serviceId",cellID);
...
var decodeXML = function(filename){
	...
	var decodeBeginDefaultVal = function(term,findAttr){			//解压入开始节点参数
		...
		$(root).find("#"+term).find(findAttr).each(function(x){
            ...
            obj.parameterName = $(this).attr("parameterName");
            obj.parameterType = $(this).attr("parameterType");
            obj.cellId = $(this).attr("cellId");
	...
	var decodeCellDefaultVal = function(cell,term,childArr){			//解压服务参数
		...
		childAttribute.map(function(x){
			...
			obj.dataType = "String";
            obj.parameterType = subChild.eq(1).attr("parameterType");
            obj.parameterDesc = subChild.eq(2).attr("parameterDesc");
			obj.id = subChild.eq(3).attr("dataID");
			obj.name = subChild.eq(4).attr("name");
            obj.uuid = subChild.eq(5).attr("uuid");
	...
	var decodeInCell = function(endId){					//将解析出的cell加入到graph中
		...
		$(root).find("#" + endId).each(function(){
			...
			if($(this).attr("label") && $(this).attr("label").split("_")[1]){
                var tempNum= $(this).attr("label").split("_")[1];

                if(Number(page.num2)<Number(tempNum)){
                    page.num2=tempNum;
				}
