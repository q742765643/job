...
import com.htht.job.executor.plugin.product.service.FloodForecastService;

@Service("floodForecastHandlerShard")
public class FloodForecastHandlerShard implements SharingHandler {

	private static Logger logger = LoggerFactory.getLogger(FloodForecastHandlerShard.class.getName());
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = floodForecastService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = floodForecastService.getIssuees(filePath, doStartTime, doEndTime, issue, null);
