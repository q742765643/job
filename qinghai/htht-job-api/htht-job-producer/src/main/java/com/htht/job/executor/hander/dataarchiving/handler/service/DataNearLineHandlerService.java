...
//			if (null == metaInfo) {
//				result.setErrorMessage("在线>近线-此dataid无效:" + handlerParam.getBackupDataid());
//				return result;
//			}
//			metaInfo.setF_nearline_time(new Date());
//			metaInfo.setF_nearlinepath(disk.getLoginurl() + "/" + srcFile.getName());
//			metaInfoService.update(metaInfo);
			
			String sql = "update htht_dms_meta_info set F_nearline_time=sysdate() , F_nearlinepath='"+disk.getLoginurl()+"/"+subDir +"\\"+ srcFile.getName()+"' where f_dataid='"+handlerParam.getBackupDataid()+"'";
			Object[] obj = {};
