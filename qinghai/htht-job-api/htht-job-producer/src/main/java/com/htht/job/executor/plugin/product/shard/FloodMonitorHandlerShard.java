...
import com.htht.job.executor.plugin.product.service.FloodMonitorService;

@Service("floodMonitorHandlerShard")
public class FloodMonitorHandlerShard implements SharingHandler {
	
	private static Logger logger = LoggerFactory.getLogger(FloodMonitorHandlerShard.class.getName());
	
	@Autowired
	private  FloodMonitorService floodMonitorService;

	@SuppressWarnings("rawtypes")
	@Override
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {

		logger.info("进入洪涝监测分片广播~");
		...
		try {
			rangeDate = floodMonitorService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = floodMonitorService.getIssuees(filePath, doStartTime, doEndTime, issue, null);
