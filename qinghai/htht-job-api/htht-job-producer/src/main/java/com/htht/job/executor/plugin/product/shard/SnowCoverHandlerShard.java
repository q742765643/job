...
import com.htht.job.executor.plugin.product.service.SnowCoverService;

@Service("SnowCoverHandlerShard")
public class SnowCoverHandlerShard implements SharingHandler {

	@Autowired
	private SnowCoverService snowcoverService;
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = snowcoverService.getRangeDate(productParam, dymap, null);//{doStartTime=Tue Dec 04 00:00:00 CST 2018, doEndTime=Thu Dec 06 23:59:59 CST 2018}
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件期号========== ***/
		List<String> issuees = snowcoverService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat);
