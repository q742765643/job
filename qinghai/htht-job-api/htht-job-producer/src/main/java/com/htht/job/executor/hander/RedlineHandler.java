...
//		VGT_NDVI_RCUR_201707240000_COFD_FY4A_AGRI_600000000000.jpg
			...
			productFileInfo.setFilePath(filePath);
...
//			productFileInfo.setRegion(regionAndFileType[0]);
			productFileInfo.setFileType(regionAndFileType[1]);
...
//			productFileInfo.setCycle(product.getCycle());
			...
			//日期匹配
			String matchIssue = RedLineMatchTime.matchIssue(info[3], product.getCycle());
//			productFileInfo.setIssue(matchIssue);
			
			
			productFileInfo.setIsDel("0");
			//如果产品已经存在，更新产品信息
			List<ProductFileInfo> existProduct = productFileInfoService.findByWhereAndRegion(product.getMark(), matchIssue, regionAndFileType);
			if (existProduct.size()>0) {
				productFileInfo.setId(existProduct.get(0).getId());
			}
			
			productFileInfoService.save(productFileInfo);
		}
