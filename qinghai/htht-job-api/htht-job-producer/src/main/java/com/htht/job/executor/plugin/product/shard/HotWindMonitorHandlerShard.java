...
@Service("hotWindMonitorHandlerShard")
public class HotWindMonitorHandlerShard implements SharingHandler {
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = hotWindMonitorService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = hotWindMonitorService.getIssuees(filePath, doStartTime, doEndTime, issue, null);
