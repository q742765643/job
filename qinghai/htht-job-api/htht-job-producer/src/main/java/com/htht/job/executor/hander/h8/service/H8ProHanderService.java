...
//	private AtomicAlgorithmService atomicAlgorithmService;
	...
	private static SimpleDateFormat ISSUE_FORMATE = new SimpleDateFormat("yyyyMMddHHmm");
...
//			return result;
		...
		for(String issue : issueList) {
			...
			try {
				Date issueDate = ISSUE_FORMATE.parse(issue);
				/** 4.拼装map **/
...
//				String outputxmlpath = "D:\\product\\h8Pro\\outputxml\\test.xml";
				...
				HashMap<String, Object> inputxmlParamMap = new HashMap<String, Object>();
...
//		}
	...
	private boolean createAlgorithmXml(String rootTag, HashMap<String, Object> inputxmlParamMap, String inputxmlpath) {
		...
		try {
            ...
            writer.write(doc);
            writer.close();
            System.out.println("写入完毕！文件位置：" + inputxmlpath);
            return true;
