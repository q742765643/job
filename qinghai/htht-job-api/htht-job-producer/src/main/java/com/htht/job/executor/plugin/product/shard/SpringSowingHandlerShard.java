...
import com.htht.job.executor.plugin.product.service.SpringSowingService;

@Service("springSowingHandlerShard")
public class SpringSowingHandlerShard implements SharingHandler{
	
	private static Logger logger = LoggerFactory.getLogger(SpringSowingHandlerShard.class.getName());
	
	@Autowired
	private SpringSowingService springSowingService;
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = springSowingService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = springSowingService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat );
