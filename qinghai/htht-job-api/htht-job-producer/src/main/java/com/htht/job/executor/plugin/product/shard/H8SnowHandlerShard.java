...
@Service("h8SnowHandlerShard")
public class H8SnowHandlerShard implements SharingHandler {
	
	private static Logger logger = LoggerFactory.getLogger(H8SnowService.class.getName());
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		String cycle = "COTM";
		try {
			rangeDate = h8SnowService.getRangeDate(productParam, dymap,cycle);//{doStartTime=Tue Dec 04 00:00:00 CST 2018, doEndTime=Thu Dec 06 23:59:59 CST 2018}
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = h8SnowService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat);
