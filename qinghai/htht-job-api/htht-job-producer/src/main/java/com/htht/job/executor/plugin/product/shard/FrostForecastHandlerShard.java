...
@Service("frostForecastHandlerShard")
public class FrostForecastHandlerShard implements SharingHandler {

	private static Logger logger = LoggerFactory.getLogger(FrostForecastHandlerShard.class.getName());
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = frostForecastService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = frostForecastService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat);
