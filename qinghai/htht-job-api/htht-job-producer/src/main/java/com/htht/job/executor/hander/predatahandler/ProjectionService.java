...
public class ProjectionService {
    ...
    public ResultUtil<String> execute(TriggerParam triggerParam, ResultUtil<String> result) {
        ...
        String inputStr = triggerParam.getExecutorParams();
        
        String inputFileStr = inputStr.split("&&")[0];
        String issue = inputStr.split("&&")[1];
        
        File dataFile = new File(inputFileStr);
        /** =======2.创建日志文件=========== **/
        ...
        try {
            if (dataFile.exists()) {
                ...
                if (!new File(outputXml).exists()) {
                    result.setErrorMessage("outputXml不存在");
                    XxlJobLogger.logByfile(outputlogpath, dataFile.getAbsolutePath() + "预处理执行失败");
                    // 释放redis
                    return result;
                }
                ...
                if (true) {
                    if (successInfo != null && successInfo.indexOf("成功") > -1) {
                        ...
                        XxlJobLogger.logByfile(outputlogpath, dataFile.getAbsolutePath() + "投影失败：" + successInfo);
                        result.setErrorMessage("投影失败");
                    }
                }
            ...
            if (redisService.exists("projection_" + redisKeyName)) {
                redisService.remove("projection_" + redisKeyName);
            }
        }
    ...
    public static String getFileDateBySatellite(String satellite, String inputFileStr) {
		
    	String issue = "";
    	if("FY4A".equals(satellite)){
    		// Z_SATE_C_BAWX_20200526162650_P_FY4A-_AGRI--_N_REGC_1047E_L1-_FDI-_MULT_NOM_20200526161918_20200526162335_1000M_V0001.HDF
    		String[] arr = inputFileStr.split("_");
    		issue = arr[4];
    	}else if("FY3D".equals(satellite)){
    		// FY3D_X_2020_04_01_03_10_D_G_MWRIX_L1B.HDF
    		String[] arr = inputFileStr.split("_");
    		issue = arr[2] + arr[3] + arr[4] + arr[5] + arr[6] + "00";
    	}else if("NOAA".equals(satellite)){
    		// NOAA18_AVHRR_2020_04_20_10_27_D_G.L1b
    		String[] arr = inputFileStr.split("_");
    		issue = arr[2] + arr[3] + arr[4] + arr[5] + arr[6] + "00";
    	}
