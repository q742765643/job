...
import com.htht.job.executor.plugin.product.service.DroughtPDIService;

@Service("droughtPDIHandlerShard")
public class DroughtPDIHandlerShard implements SharingHandler {

	@Autowired
	private DroughtPDIService droughtPDIService;
...
//	}
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = droughtPDIService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = droughtPDIService.getIssuees(filePath, doStartTime, doEndTime, issue, null);
