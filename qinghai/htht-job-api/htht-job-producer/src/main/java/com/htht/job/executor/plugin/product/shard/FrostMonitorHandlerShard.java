...
import com.htht.job.executor.plugin.product.service.FrostMonitorService;

@Service("frostMonitorHandlerShard")
public class FrostMonitorHandlerShard implements SharingHandler {

	private static Logger logger = LoggerFactory.getLogger(FrostMonitorHandlerShard.class.getName());
	
	@Autowired
	private  FrostMonitorService frostMonitorService;
	
	@SuppressWarnings("rawtypes")
	@Override
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		
		logger.info("进入霜冻监测分片广播~");
		...
		try {
			rangeDate = frostMonitorService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = frostMonitorService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat);
