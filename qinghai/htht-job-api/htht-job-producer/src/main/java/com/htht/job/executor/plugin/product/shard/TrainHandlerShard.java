...
import com.htht.job.executor.plugin.product.service.TrainService;

@Service("trainHandlerShard")
public class TrainHandlerShard implements SharingHandler {
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		try {
			rangeDate = trainService.getRangeDate(productParam, dymap, null);
		} catch (Exception e) {
		...
		/*** ======2.获取该时间段内文件的期号========== ***/
		List<String> issuees = trainService.getIssuees(filePath, doStartTime, doEndTime, issue, fileFormat);
