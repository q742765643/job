...
@Transactional
@Service("MPDIProductHandlerService")
public class MPDIHandlerService
{
	@Autowired
	...
	public ResultUtil<String> execute(TriggerParam triggerParam, ResultUtil<String> result)
	{
		try
		{
			...
			String outputxmlpath = outputxml +"\\outPutXml\\" + at.getModelIdentify() + "\\" + fileName + ".xml";
			String inputxmlpath = outputxml +"\\inPutXml\\" + at.getModelIdentify() + "\\" + fileName + ".xml";
			...
			//创建算法输出xml
			File files = new File(outputxml +"\\outPutXml\\" + at.getModelIdentify());
			if(!files.exists()){
				files.mkdirs();
			}
			// 回调需要用到
			...
			Map<String,List<Element>> outputFiles = XmlUtils.outputFilesXmlToMap(outputxmlpath);
