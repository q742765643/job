...
public class ProcessXmlParse {
    ...
    * @Date: 2018/3/26 
    */ 
    public List<ProcessSteps> parseToList(String  xmlStr, StringBuffer  startFigureBuffer) {
        ...
        try {
            ...
            //从根节点开始遍历所有节点
            this.getNodes(listElement, processStepsList, linkVos);

            for (int i = 0; i < processStepsList.size(); i++) {
                StringBuffer nextId = new StringBuffer();
                getNextId(linkVos, processStepsList.get(i).getDataId(), nextId);
                processStepsList.get(i).setNextId(nextId.toString());
            }
     ...
     * @Date: 2018/3/25
     */
    public void getNodes(List<Element> listElement, List<ProcessSteps> processStepsList, List<LinkVo> linkVos) {
        //遍历所有一级子节点
        for (Element e : listElement) {
            if (ProcessConstant.FIGUER.equals(e.getName())) {
                if(ProcessConstant.START.equals(e.attributeValue(ProcessConstant.TYPE))){
                    ...
                    processSteps.setDataId(ProcessConstant.STRATFIGURE);
                ...
                if (ProcessConstant.SERVICE.equals(e.attributeValue(ProcessConstant.TYPE))) {
                    ...
                    processSteps.setServiceId(serviceId.substring(0,serviceId.length()-5));
                    processSteps.setDataId(e.attributeValue(ProcessConstant.ID));
                    processSteps.setIsPl(e.attributeValue(ProcessConstant.ISPL));
                    processSteps.setLabel(e.attributeValue(ProcessConstant.LABEL));
                    processSteps.setIsProcess(IsProcess);
                    List<Element> next = e.elements();
                    this.getParam(next, inPutParams);
                    processSteps.setDynamicParameter(JSON.toJSONString(inPutParams));
                    processStepsList.add(processSteps);

                }
                if(ProcessConstant.END.equals(e.attributeValue(ProcessConstant.TYPE))){
                    ProcessSteps processSteps = new ProcessSteps();
                    processSteps.setDataId(ProcessConstant.ENDFIGURE);
                    List<Element> next = e.elements();
                    ...
                    processSteps.setDynamicParameter(JSON.toJSONString(commonParameterList));
                    processStepsList.add(processSteps);
                ...
                if (ProcessConstant.LINK.equals(e.attributeValue(ProcessConstant.TYPE))) {
                    ...
                    List<Element> next = e.elements();
                    this.getNodes(next, processStepsList, linkVos);
                }
