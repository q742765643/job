 ...
 */
@JobHandler(value = "generalJobHandler")
@Service
public class GeneralJobHandler extends IJobHandler {
	@Autowired
	private RedisService redisService;
	@Autowired
	private GeneralHanderService generalHanderService;
	...
	public ReturnT<String> execute(TriggerParam triggerParam) throws Exception {
		...
		/**=======2.执行业务=============================**/
		result=generalHanderService.execute(triggerParam,result);
		if(!result.isSuccess()){
			return new ReturnT<String>(ReturnT.FAIL_CODE, result.toString());
		}
