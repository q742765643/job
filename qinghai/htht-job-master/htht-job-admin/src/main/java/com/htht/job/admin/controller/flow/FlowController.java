...
public class FlowController {
    ...
    public Map<String, Object> pageList(@RequestParam(required = false, defaultValue = "0") int start,
                                        @RequestParam(required = false, defaultValue = "10") int length,FlowChartModel flowChartModel) {

        if (start != 0) {
            start = start / length;
        }
        return dubboService.pageListFlow(start, length, flowChartModel);
    ...
    public void getIcon(@PathVariable("id") String id,
        ...
        FlowChartModel flowChartModel = dubboService.getFlowById(id);
        byte[] data = flowChartModel.getProcessPicture();
    ...
    public String getProcessXmlById(@PathVariable("id") String id) {
        try {
            FlowChartModel flowChartModel = dubboService.getFlowById(id);
            Map map = new HashMap();
            map.put("xml", flowChartModel.getProcessFigure());
            map.put("id", flowChartModel.getId());
            map.put("processDescribe", flowChartModel.getProcessDescribe());
    ...
    public ReturnT<String> saveProcessFromDesigner(HttpServletRequest request) {
        try {
            ...
            dubboService.saveOrUpdateFlow(flowChartModel);
