 ...
 */
@Repository
public interface DownResultDao extends BaseDao<DownResultDTO> {

    List<DownResultDTO> findByFileNameAndFilePathAndFileSizeAndZt(String fileName, String filePath, long fileSize, String zt);
    ...
    DownResultDTO findById(String id);
    ...
    List<DownResultDTO> findByFileName(@Param("reg") String reg, @Param("startTime") String startTime, @Param("endTime") String endTime);
    ...
    List<DownResultDTO> findFilePath(@Param("reg") String reg, @Param("Time") String Time);

    @Query(nativeQuery = true, value = "SELECT CONCAT(file_path ,'/', file_name) FROM htht_cluster_schedule_download_file_info "
    		+ "WHERE file_name IN ("
    		+ "SELECT file_name FROM htht_cluster_schedule_download_file_info "
    		+ "WHERE	zt = 1 and	format = '.bz2' "
    		+ "and data_time > :startTime "
    		+ "and data_time < :endTime "
    		+ "GROUP BY left(file_name,20) HAVING COUNT(1) > 63);")
	List<String> findH8DataToProject(@Param("startTime") String startTime, @Param("endTime") String endTime);
