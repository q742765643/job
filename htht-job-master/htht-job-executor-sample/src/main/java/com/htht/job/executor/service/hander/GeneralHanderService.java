...
@Transactional
@Service("generalHanderService")
public class GeneralHanderService {
    @Autowired
    ...
    private String getDynamicMap(Map dymap, String outputxml, String outputlog, TriggerParam triggerParam, ResultUtil<String> result) {
        ...
        try {
            ...
            if (issue.indexOf("yyyy") > 0) {
                String date = DateUtil.getIssue(new Date(triggerParam.getLogDateTim()));
                issue = issue.replace("{yyyy}{mm}{dd}", date);
