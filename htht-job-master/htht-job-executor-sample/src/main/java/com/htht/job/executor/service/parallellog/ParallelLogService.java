 ...
 **/
@Service
public class ParallelLogService extends BaseService<ParallelLogDTO> {
    @Autowired
    ...
    public List<ParallelLogDTO> findParallelLogList(ParallelLogDTO parallelLogDTO) {
        SimpleSpecificationBuilder<ParallelLogDTO> specification = new SimpleSpecificationBuilder();
        Sort sort = new Sort(Sort.Direction.DESC, "createTime");

        if (!StringUtils.isEmpty(parallelLogDTO.getFlowId())) {
            specification.add("flowId", "eq", parallelLogDTO.getFlowId());
        }
        specification.add("code", "eq", parallelLogDTO.getCode());

        List<ParallelLogDTO> list = parallelLogDao.findAll(specification.generateSpecification(), sort);
        return list;
...
//		return resultList;
        ...
        for (int i = 0; i < resultList.size(); i++) {
            ...
            List<CommonParameter> all = new ArrayList<>();
            List<CommonParameter> dy = JSON.parseArray(obj[7] + "", CommonParameter.class);
            List<CommonParameter> md = JSON.parseArray(obj[8] + "", CommonParameter.class);
            if (null != dy) {
                for (CommonParameter commonParameter : dy) {
                    all.add(commonParameter);
                }
            }
            if (null != md) {
                for (CommonParameter commonParameter : md) {
                    all.add(commonParameter);
                }
            }
            result.setDynamicParameter(JSON.toJSONString(all));
