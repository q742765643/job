package com.htht.job.executor.util.monitor;
...
public class SystemInfo {
    ...
    public static Map memory(Sigar sigar) {
        ...
        try {
            ...
            monitorMap.put("jvmUse", Common.div(Double.valueOf(r.totalMemory()) - r.freeMemory(), (1024 * 1024), 2) + "M");// JVM使用内存
            monitorMap.put("jvmFree", Common.div(r.freeMemory(), (1024 * 1024), 2) + "M");// JVM剩余内存
            monitorMap.put("jvmUsage", Common.div(Double.valueOf(r.totalMemory()) - r.freeMemory(), r.totalMemory(), 2));// JVM使用率
    ...
    public static Map usage(Sigar sigar) {
        ...
        try {
            Runtime r = Runtime.getRuntime();
            monitorMap.put("jvmUsage", Math.round(Common.div(Double.valueOf(r.totalMemory()) - r.freeMemory(), r.totalMemory(), 2) * 100));// JVM使用率
            ...
            double c = 0.0;
            List<Map> disk = diskInfos(sigar);
            for (Map m : disk) {
                c += Double.valueOf(m.get(JobConstant.DISK_USAGE) + "");
            }
            monitorMap.put(JobConstant.DISK_USAGE, Math.round(c / disk.size()));// 硬盘使用率
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    public static List<Map> diskInfos(Sigar sigar) throws SigarException {
        ...
        for (int i = 0; i < fslist.length; i++) {
            ...
            // 文件系统类型名，比如本地硬盘、光驱、网络文件系统等（光驱报错）
            if (fs.getType() == 1 || fs.getType() == 2) {
