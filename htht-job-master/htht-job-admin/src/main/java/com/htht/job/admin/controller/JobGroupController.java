...
public class JobGroupController {
    ...
    public ReturnT<String> remove(int id) {
        ...
        if (count > 0) {
            return new ReturnT<>(500, "该分组使用中, 不可删除");
        }

        List<XxlJobGroup> allList = xxlJobGroupDao.findAll();
        if (allList.size() == 1) {
            return new ReturnT<>(500, "删除失败, 系统需要至少预留一个默认分组");
        }
