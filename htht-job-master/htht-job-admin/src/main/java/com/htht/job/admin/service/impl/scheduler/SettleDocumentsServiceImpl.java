...
public class SettleDocumentsServiceImpl extends FindStepServiceImpl implements SettleDocumentsService {
    ...
    public void parallelByFlowMap(ParamClassifyBean paramClassifyBean, List<CommonParameter> flowParams, List<ParallelLogDTO> parallelLogDTOList, Map methodMap) {
        ...
        ProcessStepsDTO processStepsDTO = (ProcessStepsDTO) methodMap.get(JobConstant.PROCESSSTEPS);
        if (BooleanConstant.FALSE.equals(processStepsDTO.getIsPl())) {
            ...
            this.parallelByFlowMapPl(paramClassifyBean, flowParams, parallelLogDTOList, methodMap);
    ...
    public void setFolder(List<CommonParameter> outFolder, List<CommonParameter> flowParams, Map methodMap) {
        Map mapDataId = (Map) methodMap.get(JobConstant.MAPDATAID);
        FlowChartDTO flowChartDTO = (FlowChartDTO) methodMap.get(JobConstant.FLOWCHART);
        String label = (String) methodMap.get(JobConstant.LABEL);
        outFolder.forEach(commonParameter -> {
            if (commonParameter.getValue().indexOf(flowChartDTO.getProcessCHName() + "/" + label) == -1) {
                String folder = commonParameter.getValue() + "/" + flowChartDTO.getProcessCHName() + "/" + label + "/";
    ...
    public void setFileName(String tempvalue, List<CommonParameter> outFile, List<CommonParameter> flowParams, Map methodMap) {
        ...
        String filePath = outputDirectory + File.separator + flowChartDTO.getProcessCHName() + File.separator + label + File.separator + fileName;
        ...
        outFile.forEach(commonParameter -> {
            int outIndex = (int) mapDataId.get(commonParameter.getDataID());
            flowParams.get(outIndex).setValue(outputDirectory + "/" + flowChartDTO.getProcessCHName() + "/" + label + "/" + outfilename + "/" + outfilename + flowParams.get(outIndex).getExpandedname());
