...
public class CommonParameterServiceImpl implements CommonParameterService {
    ...
    public List<CommonParameter> findOutputParameter(int jobLogId, String flowId) {
        ...
        List<FlowLogDTO> flowLogDTOList = dubboService.findFlowLogList(jobLogId, flowId);
        for (FlowLogDTO flowLogDTO : flowLogDTOList) {
    ...
    public void replaceFlowDynamicParameter(List<CommonParameter> dynamicParameter, FlowLogDTO newflowLogDTO, ParallelLogDTO parallelLogDTO, Map<Integer, String> mapValue) {
        ...
        List<CommonParameter> pdynamicParameter = JSON.parseArray(parallelLogDTO.getDynamicParameter(), CommonParameter.class);
        for (int j = 0; j < pdynamicParameter.size(); j++) {
            this.replaceFlowDynamicParameterOut(pdynamicParameter.get(j), j, newflowLogDTO, mapValue);
    ...
    public void replaceFlowDynamicParameterOut(CommonParameter commonParameter, int j, FlowLogDTO flowLogDTO, Map<Integer, String> mapValue) {
        ...
        if (FlowConstant.OUTFILE.equals(commonParameter.getParameterType()) ||
                FlowConstant.OUTSTRING.equals(commonParameter.getParameterType())) {
            if (BooleanConstant.TRUE.equals(flowLogDTO.getIsPl())) {
                String value = mapValue.get(j);
                if (StringUtils.isEmpty(value)) {
