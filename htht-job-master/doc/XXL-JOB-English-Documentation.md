...
[![Maven Central](https://maven-badges.herokuapp.com/maven-central/com.htht/xxl-job/badge.svg)](https://maven-badges.herokuapp.com/maven-central/com.htht/xxl-job/)
...
The concrete contet describe as follows:
    ...
    master.job.db.driverClass=com.mysql.jdbc.Driver
    master.job.db.url=jdbc:mysql://localhost:3306/xxl-job?useUnicode=true&characterEncoding=UTF-8
    master.job.db.user=root
    master.job.db.password=root_pwd
    
    ### Alarm mailbox
    master.job.mail.host=smtp.163.com
    master.job.mail.port=25
    master.job.mail.username=ovono802302@163.com
    master.job.mail.password=asdfzxcv
    xxl.job.mail.sendFrom=ovono802302@163.com
    master.job.mail.sendNick=《任务调度平台XXL-JOB》
    
    ### Login account
    master.job.login.username=admin
    master.job.login.password=123456
    
    ### TOKEN used for communication between the executor and schedule center, enabled if it’s not null
    master.job.accessToken=
...
The concret content of configuration file as follows:
    ...
    cluster.job.executor.logpath=/data/applogs/xxl-job/jobhandler/
...
<bean id="xxlJobExecutor" class="com.xxl.job.core.executor.XxlJobExecutor" init-method="start" destroy-method="destroy" >
    ...
    <property name="logPath" value="${cluster.job.executor.logpath}" />
...
XXL-JOB custom Quartz table structure prefix(HTHT_MASTER_SCHEDULE_).

![输入图片说明](https://static.oschina.net/uploads/img/201607/24143957_bNwm.png "在这里输入图片标题")

The added tables as shown below:
    - HTHT_MASTER_SCHEDULE_TRIGGER_GROUP:executor basic table, maintain the info about the executor;
    - HTHT_MASTER_SCHEDULE_TRIGGER_REGISTRY:executor register table, maintain addressed of online executors and schedule center machines.
    - HTHT_MASTER_SCHEDULE_TRIGGER_INFO:schedule extend table,it is used to save XXL-JOB schedule extended info,such as task group,task name,machine address,executor,input params of task and alarm email and so on.
    - HTHT_MASTER_SCHEDULE_TRIGGER_LOG:schedule log table,it is used to save XXL-JOB task’s histry schedule info,such as :schedule result,execution result,input param of scheduled task,scheduled machine and executor and so on.
    - HTHT_MASTER_SCHEDULE_TRIGGER_LOGGLUE:schedule log table,it is used to save XXL-JOB task’s histry schedule info,such as :schedule result,execution result,input param of scheduled task,scheduled machine and executor and so on.
...
```
# for cluster
org.quartz.jobStore.tablePrefix = HTHT_MASTER_SCHEDULE_
...
Task executor machine property has been canceled from v1.5, instead of task register and auto discovery, get remote machine address dynamic.
    ...
    registry table: see HTHT_MASTER_SCHEDULE_TRIGGER_REGISTRY table, it will maintain a register record periodically while task register, such as the bind relationship between machine address and AppName, so that schedule center can recognize machine list by AppName dynamicly.
...
The AccessToken of scheduler center and executor can be configured by master.job.accessToken.
...
- 1、project successfully pushed to maven central warehouse, Central warehouse address and dependency  as shown below:
    ...
    ```
- 2、To adapt to the rules of central warehouse, groupId has been changed from com.xxl to com.htht.
