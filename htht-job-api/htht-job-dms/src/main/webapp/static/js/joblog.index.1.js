...
//	}
                    ...
                    return function () {
                        if (row.triggerCode === 200){
                            ...
                            if(row.tasktype ==5){
                                ...
                                console.log(222)
                                temp = '&nbsp&nbsp<a href="javascript:;" class="logDetail" _id="'+ row.id +'" logFileName="'+ row.logFileName +'">查看日志</a>&nbsp&nbsp';

                            }

                            /*此处是针对流程日志，不做处理*/
                            if(row.tasktype===5 ||  row.tasktype===7 ||  row.tasktype===9){
    ...
    $('#joblog_list').on('click', '.logDetail', function(){
        ...
        $("#logConsoleRunning").removeClass("fa-spin");
        ...
        logConcent = "";
        $('#logConsole').html('');
        $('#logConsole').append('<span class="logText" style="display: block"></span>' +
            '<span style="color: green;display: block" class="logFinish"></span>');

        function pullLog() {
            // pullFailCount, max=20
            if (pullFailCount++ > 3) {
                $('#logConsole').append('<span style="color: red;display: block" class="logStop">终止请求Rolling日志,请求失败次数超上限,可刷新页面重新加载日志</span>');
                return;
            }
            // $('#logConsole span.logFinish').html("");
            $('#logConsole span.logText').html(" ");
            $.ajax({
                type : 'POST',
                ...
                success : function(data){
                    ...
                    if (data.code == 200) {
                        ...
                        if (fromLineNum > data.content.toLineNum ) {
                            ...
                            // valid end
                            if (data.content.end) {
                                $('#logConsole span.logFinish').html('[日志刷新完成]');
                                return;
                            }
                        }
                        ...
                        logConcent = logConcent + data.content.logContent + '<br>';
                        console.log(pullFailCount)
                        $('#logConsole span.logText').html(logConcent);
                        // pullFailCount = 0;
        ...
        //     $('#logConsole').html(content);
        // }
        $("#logConsoleRunning").on("click",function () {
            if(pullFailCount>4){
                layer.msg("无效操作", {
                    icon: 2,
                    time: 2000 //2秒关闭（如果不配置，默认是3秒）
                ...
                // $('#logConsole').html("");
                $("#logConsoleRunning").addClass("fa-spin");
                pullLog();
                layer.msg("刷新成功", {
                    icon: 1,
                    time: 2000 //2秒关闭（如果不配置，默认是3秒）
