...
define(["monitor"],function(Route){
	...
	Scoket.prototype.link = function(){
		...
		this.scoket.onmessage = function(e){
			...
			data = JSON.parse(data).msgBody;		//取出msgBody
			data=eval("("+data+")");
		...
		this.scoket.onopen = function(e){
			...
			$(".link").html("服务器已连接").css({color:"#000"});			
		}
		this.scoket.onerror = function(e){			
			$(".link").html("服务器发生错误已断开").css({color:"red"});
		}
		this.scoket.onclose = function(e){
			$(".link").html("服务器已断开,正在重新连接").css({color:"red"});
		...
		getCell : function(FigureId){		//根据ID查找出cell
			...
			switch(FigureId){
				case "startFigure":
					...
					cell.data = cell.data || {};
					this.mapData("开始节点","流程开始",cell.data);	
				break;
				case "endFigure":
					cell = page.cellEnd;
					cell.data = cell.data || {};
					this.mapData("结束节点","流程结束",cell.data);
				...
				default :
					...
					if(cell){
						cell.data = cell.data || {};
						this.mapData("普通服务",cell.value,cell.data);
					}
				break;
			}
			...
			obj.carry_state = data.status == 40 ? data.error : "已完成";
			if(data.status == 40){
	...
	showProperty.prototype = {
		showInDetail : function(){				//
            ...
            arr.map(function(x,index){
				...
				str += 	'<li style="height:'+liHeight+'px;" class="width20 odd' + index % 2 + ' border_right">' + x.parameterName + '</li>' +
