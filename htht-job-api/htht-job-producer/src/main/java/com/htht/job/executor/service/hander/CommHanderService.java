...
public class CommHanderService {
	...
	public ResultUtil<String> excute(TriggerParam triggerParam,
		...
		try {
			...
			if(StringUtils.isNotEmpty((String) fixmap.get("dataTime"))){
				date = DateUtil.strToDate((String) fixmap.get("dataTime"), "yyyy-MM-dd");
			}
			String issue = MatchTime.matchIssue(date, cycle);
			AtomicAlgorithmDTO at = atomicAlgorithmService.findModelIdentifyById(triggerParam.getAlgorId());
			...
			if(StringUtils.isNotEmpty((String) fixmap.get("executMode")) && "1".equals((String) fixmap.get("executMode")) ){
				...
				Map<String, List<Element>> outputFiles = XmlUtils.outputFilesXmlToMap(outputxml);
				List<String> regionIdList = XmlUtils.getXmlAttrVal(outputxml, "region", "identify");

				List<String> mosaicFiles = XmlUtils.getXmlAttrFileElementVal(outputxml, "mosaicFile");
				String mosaicFile = "";
				...
				if(regionIdList!=null&&regionIdList.size()>0) {
					for(String regionId:regionIdList) {
						// 产品信息及文件信息入库
						ProductInfoDTO productInfoDTO = productUtil.saveProductInfo(
