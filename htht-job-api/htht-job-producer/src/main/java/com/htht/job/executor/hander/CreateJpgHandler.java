...
public class CreateJpgHandler extends IJobHandler {
	...
	private ResultUtil<String> copyTootherFolders(String fromPath, String toPath, String type,
		...
		try {
			...
			String startPath = startTempFile.getParent() + File.separator + FileUtil.getNoFileExtensionName(startTempFile);
			
			File fromFile = new File(fromPath);
			String[] fromPathMs = { fromFile.getParent() + File.separator + FileUtil.getNoFileExtensionName(fromFile) + ".xml",
					fromFile.getParent() + File.separator + FileUtil.getNoFileExtensionName(fromFile) + ".jpg" };
			String[] fromPathPs = { fromFile.getParent() + File.separator + FileUtil.getNoFileExtensionName(fromFile).replace("MSS", "PAN") + ".xml",
					fromFile.getParent() + File.separator + FileUtil.getNoFileExtensionName(fromFile).replace("MSS", "PAN") + ".jpg" };
			...
			for (String endString : endPath) {
				...
				for (String from : fromPaths) {
					try (BufferedInputStream bin = new BufferedInputStream(new FileInputStream(from));
							// 输出目录
							BufferedOutputStream bout = new BufferedOutputStream(
									new FileOutputStream(endString + FileUtil.getFileExtensionName(from)));) {
	...
	private void getActualParameters(LinkedHashMap dymap) {
		...
		String projectName = FileUtil.getNoFileExtensionName(projectPathFile);
