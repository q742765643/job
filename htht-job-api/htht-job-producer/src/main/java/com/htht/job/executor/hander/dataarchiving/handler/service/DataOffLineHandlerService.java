...
public class DataOffLineHandlerService {
	...
	public ResultUtil<String> excute(TriggerParam triggerParam, ResultUtil<String> result) {
		try {
			...
			if (subDir.size() == 0) {
				result.setErrorMessage("近线>离线-根据数据ID查询备份路径目录失败，数据ID("+handlerParam.getBackupDataid()+")无效！");
				return result;
			}
			...
			String sql = "update htht_dms_meta_info set F_offline_time=sysdate() , F_offlinepath='"+disk.getLoginurl()+"/"+subDir.get(0)+"\\"+ srcFile.getName()+"' where f_dataid='"+handlerParam.getBackupDataid()+"'";
			Object[] obj = {};
