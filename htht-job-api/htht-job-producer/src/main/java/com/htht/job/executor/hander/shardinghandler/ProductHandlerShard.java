...
public class ProductHandlerShard implements SharingHandler
{
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap)
	{
		...
		{
			startDate = com.htht.job.core.util.DateUtil.getCurrentDateStringWithOffset(Consts.DateForMat.yyMMddFormatSplited,
					-1 * Integer.parseInt(productParam.getProductRangeDay()));
			endDate = com.htht.job.core.util.DateUtil.getCurrentDateString(Consts.DateForMat.yyMMddFormatSplited);
		} else
		{
		...
		{
			com.htht.job.core.util.MatchTime matchTimeUtil = new com.htht.job.core.util.MatchTime();
