 ...
 *  产品标识：identify
 */
@Service("standardProductHandlerShard")
public class StandardHandlerShard implements SharingHandler {
	...
	public ResultUtil<List<String>> execute(String params, LinkedHashMap fixmap, LinkedHashMap dymap) throws Exception {
		...
		if (proMark.equals("scanfolder")) {
			...
			List<File> fileList = FileUtil.iteratorFileAndDirectory(new File(inputFolder), filePattern);
