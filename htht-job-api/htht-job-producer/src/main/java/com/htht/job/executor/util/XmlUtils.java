...
public class XmlUtils {
    ...
    private static Map<String,List<Element>> res = new HashMap<>();
     ...
     * @return:
     * @date: 2018/9/28
     * @update: 2019/4/26 修正线程不安全问题（HHui）
     */
    public static synchronized List<String> getXmlAttrFileElementVal(String path, String elementName,String identify,String regionId) {
    	Map<String,List<Element>> map = outputFilesXmlToMap(path);
     ...
     * @update: 2019/4/26 修正线程不安全问题（HHui）
     */
    public static synchronized List<Element> getTablenameElements(String path,String tableName) {
        List<Element> lists = new ArrayList<>();
        List res = (List)tablesXmlToMap(path).get(tableName);
        if(res.size() > 0){
